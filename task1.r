...
# Calculate Sensitivity, Precision, Etc
...
PRROC_obj <- roc.curve(scores.class0 = data$Rater1, weights.class0=data$Patient_Diagnosis,
                       curve=TRUE)
plot(PRROC_obj)

PRROC_obj <- roc.curve(scores.class0 = data$Rater2, weights.class0=data$Patient_Diagnosis,
                       curve=TRUE)
plot(PRROC_obj)
...
model <- glmer(Rating ~ Patient_Diagnosis + (1 | X) + (1 | Rater),
               data = df_long,
               family = binomial)
summary(model)
